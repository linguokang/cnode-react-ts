...
interface Imessage {
    [x: string]: any;
    has_read_messages: any[],
    hasnot_read_messages: any[],
}
...
@inject('store')
@observer class Messages extends Component<IProps,IState> {

    @observable message: Imessage = {         // 已读和未读消息
        has_read_messages: [],
        ...
        var Msg = ( obj:any ) => {
            return (
                <div className="msg-list">
                    ...
                    { this.message[obj] && this.message[obj].length === 0 && <div className="no-msg">暂无消息</div> }
                    <ul>
                    {
                        this.message[obj] && this.message[obj].map((item: any, index: number) => {
                            return (
                                <li key={item.id}>
                                    {
                                        item.type === 'reply' ?
